...
class NeuronGatedDeltaNet(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        # recurrent_state is kept in fp32 in HBM: the per-step updates are
        # fp32 inside the NKI kernel, and for long prefill (~1000+ valid
        # tokens, e.g. 1024x1024 VL) the state magnitude grows enough that
        # casting back to bf16 loses precision that TKG then amplifies into
        # immediate EOS. Extra memory cost is 512 KB per DeltaNet layer,
        # < 10 MB total, so keep the full precision.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Output: gated RMSNorm, then project. Match HF Qwen3_5RMSNormGated
        # (transformers.models.qwen3_5.modeling_qwen3_5.Qwen3_5RMSNormGated):
        # keep norm math and silu(gate) in fp32 so that long-prefill VL
        # (1k+ tokens with vision embedding norms ~10x text) doesn't lose
        # precision through an early bf16 cast. The PR's original code cast
        # `output` to bf16 before the norm and computed silu(gate) in bf16;
        # that is fine for short prompts but blows up the 1024-merged-token
        # VL case because the recurrent-state-derived `output` has magnitude
        # ~1e3 after that many steps and needs its fp32 mantissa.
        ...
        # Manual RMSNorm in fp32 (equivalent to self.norm but guaranteed fp32)
        variance = output_fp32.pow(2).mean(-1, keepdim=True)
        eps = getattr(self.norm, "variance_epsilon", 1e-6)
        output_fp32 = output_fp32 * torch.rsqrt(variance + eps)
        weight = self.norm.weight.float() if self.norm.weight is not None else None
        if weight is not None:
            output_fp32 = output_fp32 * weight

        # Gate in fp32, cast back to model dtype at the end.
        z_gate = z.reshape(batch_size, seq_len, self.num_v_heads, self.head_v_dim)
        output_fp32 = output_fp32 * F.silu(z_gate.to(torch.float32))
