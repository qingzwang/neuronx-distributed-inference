...
def deltanet_fused_chunked_fwd(
    ...
    for i_chunk in nl.sequential_range(num_chunks):
        ...
        # No clamp here: downstream, all exp(gc...) sites operate on the
        # log-sum-exp-reduced form (gc[i]-gc[j]) or on gc clamped to <=0,
        # which keeps every exp in [0, 1] regardless of |gc|.
        ...
        # ============================================================
        # Log-sum-exp-style decay matrix.
        #
        # The original code computed decay[i,j] = exp(gc[i]) * exp(-gc[j])
        # as two separate broadcasts. Because gc is a cumsum and can
        # reach ~1e3 in magnitude for some heads, exp(-gc[j]) overflows
        # to inf in fp32 long before the lower-triangular mask can zero
        # it out.
        #
        # Equivalent formulation:
        #   decay[i,j] = exp(gc[i] - gc[j])
        # Since the result is later multiplied by a strict-lower-tri mask
        # (valid entries have i > j and gc is non-increasing, so
        # gc[i] - gc[j] <= 0), we can safely clamp the diff to (-inf, 0]
        # before exp. That makes decay[i,j] in [0, 1] on the valid side
        # and keeps all intermediates finite regardless of |gc|.
        #
        # We still need exp(gc[i]) as a (P_MAX, 1) tensor for the two
        # sites that multiply by it alone (k_beta * exp(gc) and
        # q * exp(gc)). For those we clamp gc to <= 0 inside the chunk
        # frame so exp is in (0, 1]; this holds exactly when g <= 0,
        # which is the DeltaNet convention (g = -A * softplus(...)).
        # ============================================================

        # gc_row_bcast: (P_MAX, P_MAX) where every row is gc^T (broadcast
        # partition 0 of gc_row to all partitions).
        gc_row_bcast = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        for i_shuf in nl.static_range(P_MAX // 32):
            nisa.nc_stream_shuffle(
                src=gc_row[0:1, 0:CHUNK_SIZE],
                dst=gc_row_bcast[i_shuf * 32 : i_shuf * 32 + 32, 0:CHUNK_SIZE],
                shuffle_mask=_BROADCAST_MASK,
            )

        # gc_col_bcast[i, j] = gc[i] (constant along free dim), built from
        # gc_p via tensor_scalar(multiply, ones).
        ...
        # diff_safe = min(diff, 0) so exp stays in [0, 1]. Entries in the
        # upper triangle are about to be masked away anyway.
        ...
        # decay_mat[i, j] = exp(gc[i] - gc[j]) in [0, 1]
        ...
        # For k_beta * exp(gc) and q * exp(gc) we need exp(gc) as a
        # (P_MAX, 1) scalar. Clamp gc to <= 0 first (holds exactly when
        # g <= 0, which is the DeltaNet convention) so exp is safely in
        # (0, 1].
        ...
        # exp(g_last): scalar, clamped to (-inf, 0] for the same reason,
        # then broadcast to (P_MAX, 1).
        gl_nonpos_11 = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_scalar(
            dst=gl_nonpos_11,
            data=gl_11,
            op0=nl.minimum,
        ...
        # ============================================================
        # QK_decay[i,j] = QK[i,j] * decay_mat[i,j]
        # (Was two row/col broadcasts with exp(gc) * exp(-gc); replaced
        # by a single tensor_tensor multiply against the pre-built decay
        # matrix. See "Log-sum-exp-style decay matrix" above.)
        # ============================================================
        ...
        # qk_decay[i,j] = qk_raw[i,j] * decay_mat[i,j]
        # (Same log-sum-exp rewrite as QK_decay above -- reuse decay_mat.)
        qk_decay = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_tensor(
        ...
        #
        # Original code factored exp(gl) out:
        #   state = exp(gl) * (state + (k * exp(-gc))^T @ v_new)
        # which hits the same exp(-gc) overflow as the decay mask above.
        # Rewriting with exp(gl - gc) keeps every exponent in (-inf, 0],
        # because gl = gc[last] and gc is non-increasing (g <= 0), so
        # gl - gc[t] <= 0 for all t.  We also clamp to max=0 defensively
        # for the rare head where the sign assumption fails.
        # ============================================================

        # Broadcast gl_11 from (1, 1) to (P_MAX, 1) so tensor_scalar can
        # use it as a partition-aligned operand against gc_p.
        ...
        # state = state_decayed + kv_outer
        nisa.tensor_tensor(
            dst=state, data1=state_decayed, data2=kv_outer, op=nl.add
        )
