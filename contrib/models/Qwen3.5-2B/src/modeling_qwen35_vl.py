...
class NeuronQwen35VLForCausalLM:
    ...
    def generate(
        ...
        **kwargs,
        ...
        _timings = {} if return_timings else None
        ...
        if has_vision and self.vision_model_wrapper is not None:
            ...
            _t_ve_start = _time.perf_counter()
            vision_embeddings = self.vision_model_wrapper(pixel_values, image_grid_thw)
            if _timings is not None:
                _timings["vision_encode_ms"] = (
                    _time.perf_counter() - _t_ve_start
                ) * 1000.0
        ...
        next_token = self._extract_next_token(
            output, temperature, top_p, top_k
        )
        if _timings is not None:
            # TTFT measured from the start of generate() -- includes vision
            # encode, mRoPE build, and the CTE prefill forward. This is what
            # a user experiences as "time to first token".
            _timings["ttft_ms"] = (
                _time.perf_counter() - _t_generate_start
            ) * 1000.0
            _timings["cte_ms"] = (
                _time.perf_counter() - _t_cte_start
            ) * 1000.0
        ...
        if next_token.item() in eos_token_ids:
            if _timings is not None:
                _timings["tkg_ms"] = 0.0
                _timings["tkg_tokens"] = 0
                _timings["total_ms"] = (
                    _time.perf_counter() - _t_generate_start
                ) * 1000.0
                _timings["new_tokens"] = 1
                return generated_ids, _timings
            return generated_ids

        # Step 4: Token generation (TKG) loop
        _t_tkg_start = _time.perf_counter()
        _tkg_count = 0
        ...
        if _timings is not None:
            _timings["tkg_ms"] = (
                _time.perf_counter() - _t_tkg_start
            ) * 1000.0
            _timings["tkg_tokens"] = _tkg_count
            _timings["total_ms"] = (
                _time.perf_counter() - _t_generate_start
            ) * 1000.0
            _timings["new_tokens"] = 1 + _tkg_count
            return generated_ids, _timings
        return generated_ids

    def _extract_next_token(self, output, temperature, top_p, top_k):
        """Extract the next token from text_model output.

        When on_device_sampling is enabled, `output.logits` is None and the
        sampled token IDs are on `output.tokens` (CausalLMOutputWithPast
        constructed by NeuronBaseForCausalLM). Otherwise `output.logits`
        holds the logits and we sample on the CPU side.
        """
        if isinstance(output, tuple):
            primary = output[0]
        elif getattr(output, "logits", None) is None and hasattr(output, "tokens"):
            primary = output.tokens
        else:
            primary = output.logits

        if primary.dtype in (torch.int32, torch.int64):
            # Sampled token IDs, shape (B,) or (B, 1) or (B, 1, 1)
            flat = primary.reshape(primary.shape[0], -1)
            return flat[:, -1]
        # Logits path: shape (B, seq_len, V) -- sample the last position
        return self._sample_token(primary[:, -1, :], temperature, top_p, top_k)
