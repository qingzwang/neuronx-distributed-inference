...
## Sample detections (yolo26n FP32 on Neuron)

Both images run through the compiled Neuron model, top-k + letterbox-unmap
on CPU, and the final boxes overlaid with class + score. Generated by
`src/visualize.py`:

| bus.jpg | zidane.jpg |
| :---: | :---: |
| ![bus](assets/bus_pred.jpg) | ![zidane](assets/zidane_pred.jpg) |
| 1 bus + 4 persons (scores 0.53–0.93) | 2 persons + 1 tie (scores 0.53–0.92) |
...
yolo26/
  src/
    ...
    visualize.py          # run Neuron on sample images, draw boxes + class + score
  test/
...
# 8. Render detection boxes + class + score onto every sample image in assets/.
python visualize.py                  # uses compiled/yolo26n_neuron_fp32.pt

# 9. Unit tests (skip automatically if Neuron / compiled .pt absent)
